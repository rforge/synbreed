...
\SweaveOpts{prefix.string=figs/synbreed}
...
This data set contains genotypic, phenotypic, map and pedigree data of 500 bulls. Genotypic and pedigree data is based on  a real cattle data set while phenotypes were simulated artificially. The \code{cattle} data was provided by the Animal Breeding and Genetics group of Henner Simianer, Georg-August-Universität Göttingen. The data can be loaded using
...
\caption{LD  decay as a scatter plot (left side) and stacked histogram (right side) for \Sexpr{nrow(maize$map[maize$map$chr==1,])} markers on the first chromosome for the \code{maize} data.}
\label{fig:LDDist}
\end{figure}   
