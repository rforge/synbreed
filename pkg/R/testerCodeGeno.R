...
testerCodeGeno <- function(gpData,impute=FALSE,impute.type=c("family","beagle","beagleAfterFamily","random","fix"),
                       replace.value=NULL,maf=NULL,nmiss=NULL,label.heter="AB",keep.identical=TRUE,verbose=FALSE,tester=NULL){
  ...
  if (verbose) cat("Begin   :",ncol(data),"marker(s) were in the data set for ", n, " Genotypes at the begin\n")
  #============================================================
  ...
  #============================================================
  # step 2a  - remove markers for which the tester is not homozygous or values missing (optional, argument tester = "xxx")
  #============================================================
  ...
  if(!is.null(tester)){
    ...
    dataRaw <- dataRaw[,which.miss]
    if(sum(!which.miss) > 0){
      if (verbose) cat("step 2a :",sum(!which.miss),"marker(s) removed because heterozygousity at tester locus or \n          missing values of the tester\n")
    } else{
      if (verbose) cat("step 2a : No marker(s) removed because heterozygousity at tester locus or \n          missing values of the tester\n")
    }
    cnames <- cnames[which.miss]
    # update map
    if(!is.null(gpData$map)) gpData$map <- gpData$map[which.miss,]
  ...
  res[res == label.heter] <- 1
  res <- matrix(as.numeric(res),nrow=n)

  #============================================================
  # step 2b  - remove markers for which the tester has the minor allele
  #============================================================

  if(!is.null(tester)){
    which.miss <- res[rnames==tester,] != 3
    dataRaw <- dataRaw[,which.miss]
    res <- res[,which.miss]
    if(sum(!which.miss) > 0){
      if (verbose) cat("step 2b :",sum(!which.miss),"marker(s) removed for which the tester has the minor allele\n")
    } else{
      if (verbose) cat("step 2b : No marker(s) removed for which the tester has the minor allele\n")
    }
    cnames <- cnames[which.miss]
    # update map
    if(!is.null(gpData$map)) gpData$map <- gpData$map[which.miss,]
  ...
  #============================================================
  # step 2c  - remove homozygout values of the minor allele    
  #============================================================
  ...
  if(!is.null(tester)){
    ...
    if(!is.null(nmiss)){
      ...
      res <- res[,which.miss]
      if (verbose) cat("step 2c :",sum(!which.miss),"marker(s) removed with >",nmiss*100,"% false genotyping values \n")
      cnames <- cnames[which.miss]
      # update map
      if(!is.null(gpData$map)) gpData$map <- gpData$map[which.miss,]
    } else{
      if (verbose) cat("step 2c : No markers removed due to fraction of missing values \n")
    }
    sum(is.na(res))
    sum(!is.na(res))
    dim(res)
  ...
  # number of markers
    M <- ncol(res)
    if(M == 0) stop("No remaining markers due to too many missing values")
...
#set.seed(19810703)
for(j in 1:5){
mat.gen <- matrix(sample(c("AA","AA","BB",NA), prob = c(.225,.45,.225, .1), size = 1000, replace = TRUE), ncol = 25, nrow = 40)
colnames(mat.gen) <- paste("M", 1:ncol(mat.gen), sep = "")
rownames(mat.gen) <- paste("G", 1:nrow(mat.gen), sep = "")
vec.famil <- rep(1:8, each = 5)
names(vec.famil) <- rownames(mat.gen)

gpTest <- create.gpData(family = as.matrix(vec.famil), geno = mat.gen)
try(gpTestCode <- testerCodeGeno(gpTest, impute = TRUE, impute.type = "family", maf = 0.05, nmiss = 0.1, verbose = TRUE, label.heter = NULL#, tester = "G1")
))
print(gpTestCode$geno)
print(mat.gen[, colnames(gpTestCode$geno)])
}
