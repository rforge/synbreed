manhattanPlot <- function(b,gpData=NULL,colored=FALSE,add=FALSE,pch=19,ylab=NULL,cols=NULL,...){
  if(is.null(gpData)) plot(b,...)
    ...
    if(is.null(cols)){
      ...
      cols=cols[(as.numeric(gpData$map$chr)-1)%%6+1]
    }
    ...
    if(!add){
      plot(chr,b,col=cols,type="p",axes=FALSE,pch=pch,ylab=ylab,cex.axis=.9,...)
