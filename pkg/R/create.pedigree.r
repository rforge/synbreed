...
add.pedigree <- function(ped,IDped,gener=NULL,sex=NULL,add.ancestors=FALSE){
  if(!all.equal(class(ped), c("pedigree", "data.frame"))) stop(paste(substitute(ped), "has to be an object generated by create.pedigree()"))
  n <- nrow(IDped)
  # only unique IDs
  if(length(unique(IDped[,1]))!=n)  warning("IDs are not unique, removing duplicated individuals")
