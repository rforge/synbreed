...
In section \ref{sec:RelationshipMatrices} several methods to set up relationship matrices are presented. Section \ref{sec:LinkageDisequilibrium} shows how Linkage Disequilibrium between markers could be expoited using \texttt{synbreed} package.
