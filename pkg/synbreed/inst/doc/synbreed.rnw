...
data(maize)
@
This data set contains genotypic and phenotypic data, as well as pedigree information up to grandparents for \Sexpr{ nrow(maize.pheno)} doubled haploid (DH) lines of maize. Performance of DH lines was evaluated in test crosses with a common single cross tester. When loading \texttt{maize} data, the following
...
In the first step, genotypes have to be coded in a way that it could be used for the construction of genomic relationship matrices. The convention used in this package is to code genotypes by their number of minor alleles, i.e. 0 for one homozygous genotypes and 2 for the other and 1 for heterozygous.
...
\textit{Linkage Disequilibrium} (LD) is defined as a non-random association between polymorphisms of two or more molecular markers (usually part of same linkage group). Average LD over the whole genome is important as it influences prospects of genomic selection. 
...
For vanRaden, the SNP genotypes are coded as the number of copies of one of the minor allele, i.e., 0, 1 or 2 (no missing values allowed). Thus the marker data could be the result of a call of \texttt{codeGeno} when imputing for the missing values was performed or the missing values were replaced with the value 1.
...
Variance components are specified in a separate list and values used for simulation are $\sigma^2_a=10$, $\sigma^2=15$ and $0$ for block and location. Simulated data is obtained as follows
<<>>=                                      
...
summary(mod)           
@
Note that variance-covariance matrix must be of same dimension as $y$. This could easily be obtained by using the Kronecker product to enlarge relationship matrix as data is sorted by individuals. This is shown in the code above. As the fitted model only contains one random effect, estimated breeding values are 
<<>>=
...
@
Estimates for variance components and fixed effects are shown in the summary of the models. Table \ref{tab:mods} compares the three models
\begin{table}[h]
\centering
